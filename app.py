...
st.set_page_config(
    layout="wide",
    page_title="Smart Schmidt Hammer AI System V38.0",
...
API_KEYS = {
    "ROBOFLOW_API": get_secret_value("ROBOFLOW_API", ""),
    "KMA_WEATHER": get_secret_value("KMA_WEATHER", ""),
    "GEMINI_API": get_secret_value("GEMINI_API", ""),
}
...
def create_page1_pdf(data):
    ...
    meta_text = f"""
    <b>슈미트 희망 날짜:</b> {pdf_safe_text(data.get('date'))} {pdf_safe_text(data.get('time'))}<br/>
    <b>슈미트 타격 물리적 위치:</b> {pdf_safe_text(data.get('loc'))}<br/>
    <b>희망 타격 횟수:</b> {pdf_safe_text(data.get('target'))}회<br/><br/>
    <b>기상청 실시간 API 기반 온도 및 습도:</b> 온도({pdf_safe_text(data.get('temp'))}℃), 습도({pdf_safe_text(data.get('hum'))}%)<br/>
    <b>[이에 따른 해당 날짜 및 시간, 장소에 슈미트해머 치기 [{pdf_safe_text(data.get('weather_ok'))}]합니다.]</b>
    """
    story.append(Paragraph(meta_text, body_style))
    story.append(Spacer(1, 8))

    ai_text = f"""
    <b>선택 AI:</b> 균열/철근노출탐지 AI ({pdf_safe_text(data.get('ai_1'))}), 요철/불균질면 탐지 AI ({pdf_safe_text(data.get('ai_2'))}), 범용 콘크리트 결함 AI ({pdf_safe_text(data.get('ai_3'))}), 네이버/아마존/구글/자체 AI (X - 추후 연동 예정)<br/>
    <b>선택 AI 연동 주소:</b> {pdf_safe_text(data.get('ai_urls'))}
    """
    story.append(Paragraph(ai_text, body_style))
    story.append(Spacer(1, 8))

    cal_text = f"""
    <b>기준점 위치:</b> {pdf_safe_text(data.get('pts'))}<br/>
    <b>기준점 실제 거리:</b> {pdf_safe_text(data.get('dist'))} mm<br/>
    <b>기준점에 근거한 사진 찍은 벽면의 실제 가로x세로는:</b> {float(data.get('area', 0)):.1f} cm² 이며 픽셀당 거리는 {float(data.get('px_scale', 0)):.4f} cm 입니다.
    """
    story.append(Paragraph(cal_text, body_style))
    story.append(Spacer(1, 8))

    def cv2_to_rl(img_cv, w_img, h_img):
        if img_cv is None:
            return Paragraph("[이미지 없음]", table_text)
        try:
            img = img_cv.copy()
            if len(img.shape) == 3:
            ...
            return RLImage(img_buf, width=w_img, height=h_img)
        except Exception:
            return Paragraph("[이미지 오류]", table_text)

    img_row1 = [
        Paragraph("<b>[선택 사진 1: AI 불균질 검출망]</b>", table_text),
        Paragraph("<b>[선택 사진 2: 최종 추천 타격점]</b>", table_text)
    ]
    img_row2 = [
        cv2_to_rl(data.get("img_map"), 240, 240),
        cv2_to_rl(data.get("img_strike"), 240, 240)
    ]
    ...
    ai_comment = pdf_safe_text(data.get("ai_comment", ""))

    res_text = f"""
    <b>분석 근거 &amp; 코멘트:</b><br/>
    확보된 타격점: {pdf_safe_text(data.get('cand_count'))}개 
    결함 의심 비율: {float(data.get('defect_ratio', 0)):.2f}%<br/>
    {ai_comment}
    """
...
def create_page2_pdf(data):
    ...
    meta_text = f"""
    <b>타격 날짜:</b> {pdf_safe_text(data.get('date'))} {pdf_safe_text(data.get('time'))}<br/>
    <b>타격 장소:</b> {pdf_safe_text(data.get('loc'))}<br/>
    <b>기상청 실시간 API 기반 온도 및 습도:</b> 온도({pdf_safe_text(data.get('temp'))}℃), 습도({pdf_safe_text(data.get('hum'))}%)<br/>
    <b>[이에 따른 해당 날짜 및 시간, 장소에 슈미트해머 치기 [{pdf_safe_text(data.get('weather_ok'))}]합니다.]</b><br/><br/>
    <b>타격 횟수:</b> {pdf_safe_text(data.get('strike_count'))}회<br/>
    <b>콘크리트 타설일:</b> {pdf_safe_text(data.get('pour_date'))} - 해당 날짜로부터 오늘까지 {pdf_safe_text(data.get('days'))}일이 경과하였습니다. 재령일: {pdf_safe_text(data.get('days'))}일<br/>
    <b>설계기준강도:</b> {pdf_safe_text(data.get('fck'))} MPa<br/>
    <b>선택 초음파 속도:</b> {pdf_safe_text(data.get('ultra'))}<br/>
    <b>선택 슬럼프:</b> {pdf_safe_text(data.get('slump'))}<br/>
    <b>타격 각도:</b> {pdf_safe_text(data.get('angle'))}°
    """
    story.append(Paragraph(meta_text, body_style))
    story.append(Spacer(1, 10))

    r_vals = data.get("r_list", [])
    chunk_size = 10

    for i in range(0, len(r_vals), chunk_size):
        chunk = r_vals[i:i + chunk_size]
        r_headers = [
            Paragraph(f"<b>현장 반발도 {i + j + 1}번째</b>", table_text)
            for j in range(len(chunk))
        ]
        r_values = [
            Paragraph(pdf_safe_text(val), table_text)
            for val in chunk
        ]

        col_width = 540 / max(1, len(chunk))
        t_r = Table([r_headers, r_values], colWidths=[col_width] * len(chunk))
    ...
    res_data = [
        [
            Paragraph("<b>전체 반발도 평균</b>", table_text),
            Paragraph(f"{float(data.get('r_mean', 0)):.2f} R", table_text),
            Paragraph("<b>KS 보정 반발도</b>", table_text),
            Paragraph(f"{float(data.get('r_corr', 0)):.2f} R", table_text)
        ],
        [
            Paragraph("<b>보정 반발도 활용 강도</b>", table_text),
            Paragraph(f"{float(data.get('fc_reb', 0)):.1f} MPa", table_text),
            Paragraph("<b>초음파 강도</b>", table_text),
            Paragraph(f"{float(data.get('fc_ultra', 0)):.1f} MPa", table_text)
        ],
        [
            Paragraph("<b>최종 복합 강도</b>", table_text),
            Paragraph(f"<b>{float(data.get('fc_hybrid', 0)):.1f} MPa</b>", table_text),
    ...
    t_res.setStyle(TableStyle([
        ...
        ("INNERGRID", (0, 0), (-1, -1), 0.5, colors.HexColor("#90CDF4")),
    ...
    - <b>복합강도식:</b> Fc = (1.52 x R0 + 16.87 x V - 66.9) x 환경/재령/슬럼프 보정계수<br/>
    - <b>학술 근거:</b> 초음파-반발도 데이터 융합 및 슬럼프 공극률 보정 계수 모델링
    """
...
작성 조건: 한국어, KS F 2730 이상치 정제 의미 및 복합법 연동 설명, 5~7문장.
현장 데이터:
{data_summary}
"""
...
if "1." in main_menu:
    ...
    if is_weather_valid:
        st.success("✅ [적절] 온도와 습도가 허용 범위 내에 있어 측정 신뢰성이 높습니다.")
    else:
        st.error("❌ [부적절] 온도가 5~35℃를 벗어나거나 습도가 80% 이상입니다.")
    ...
    if uploaded_file:
        ...
        st.success(
            f"📊 **기준점 기반 실제 규격 분석 완료:** 사진 속 콘크리트 측정면의 실제 가로세로는 "
        ...
        if is_usable:
            st.success(
                f"⚙️ **KS 표준 기반 희망 타격 횟수 확보 성공:** 요청한 `{desired_strikes}개`의 주 타격점이 모두 "
                f"결함 회피 영역, 경계부 이격 조건, 타격점 간 최소 3cm 이격 조건을 만족하도록 선정되었습니다."
            )
        else:
            st.error(
                f"❌ **희망 타격 횟수 확보 실패:** 현재 영상에서 안전 타격 후보점은 `{final_selected_count}개` 수준으로 확인되며, "
        ...
        if is_weather_valid and is_usable:
            ...
            reliability_msg = "환경 조건과 표면 조건이 모두 양호하여 측정 신뢰도가 높습니다."
        elif is_usable:
            reliability_grade = "보통"
            reliability_msg = "타격 영역은 확보되었으나 온도/습도 주의가 필요합니다."
        else:
            reliability_grade = "낮음"
            reliability_msg = "타격 가능 영역이 부족합니다. 표면 정리 후 재분석을 권장합니다."

        st.markdown("#### 🧾 AI 표면 신뢰도 판정 요약")
        st.info(
            f"**분석 대상 면적:** `{real_width_cm:.1f} cm × {real_height_cm:.1f} cm = {calculated_area_cm2:,.1f} cm²`  \n"
            f"**픽셀 환산 계수:** `1 pixel = {p_scale_cm:.4f} cm`  \n"
            f"**AI 결함 의심 비율:** `{defect_ratio:.2f} %`  \n"
            f"**확보된 후보 타격점:** `{final_selected_count}개` / 요청 `{desired_strikes}개`  \n"
            f"**종합 신뢰도 등급:** `{reliability_grade}`  \n\n"
            f"{reliability_msg}"
        )
        ...
        if st.button("🚀 1페이지 Gemini AI 분석 코멘트 생성"):
            with st.spinner("Gemini AI가 분석 중입니다..."):
                p1_summary = (
                    f"측정위치:{m_loc}, 일시:{m_date} {selected_time}, 온도:{auto_temp}℃, 습도:{auto_hum}%, "
                    f"결함비율:{defect_ratio:.2f}%, 후보점:{final_selected_count}개"
                )
        ...
        st.caption("버튼을 누르면 현재 연산된 이미지, 데이터, 분석 코멘트가 매핑된 1페이지 전용 PDF 보고서가 생성됩니다.")

        p1_pdf_data = {
            "date": str(m_date),
            "time": selected_time,
            "loc": m_loc,
            "target": desired_strikes,
            "temp": auto_temp,
            "hum": auto_hum,
            ...
            "dist": real_len,
            "area": calculated_area_cm2,
            "px_scale": p_scale_cm,
            "img_map": weather_map_img,
            "img_strike": strike_map_img,
            "defect_ratio": defect_ratio,
            "cand_count": final_selected_count,
    ...
    with col_env:
        ...
        if use_ultra:
            ...
            hz_spec = st.selectbox(
                "📡 센서 주파수 대역 (학술 보고용)",
                ["54 kHz (토목/콘크리트 표준)", "150 kHz (정밀/소형)", "24 kHz (대형 구조물)"]
            )
        else:
    ...
    with col_data:
        ...
        with c_strk2:
            ...
            angle_opts = [
                f"{a}° (상향/천장)" if a > 0 else f"{a}° (하향/바닥)" if a < 0 else f"{a}° (수평/벽면)"
                for a in angles
            ]
    ...
    total_avg = float(np.mean(raw_arr))
    ...
    ks_avg = float(np.mean(filtered_data)) if filtered_data else total_avg
    ...
    if fck > 0:
        design_ratio = fc_final_hybrid / fck * 100
    else:
        design_ratio = 0
    ...
    st.info(
        ...
        f"(설계기준강도 {fck} MPa 대비 {design_ratio:.1f}% 수준)"
    )
    ...
    with c_lx2:
        if use_ultra:
            ...
            st.latex(
                r"F_c = \left[ 1.52 \cdot R_0 + 16.87 \cdot V_{(km/s)} - 66.9 \right] \times f_{age} \times f_{env} \times f_{slump}"
            )
        else:
            st.caption("✔️ **[대한건축학회] 단일 반발도 추정 선형식**")
            st.latex(
                r"F_c = \left[ 1.3 \cdot R_0 - 14.0 \right] \times f_{age} \times f_{env} \times f_{slump}"
            )
...
""")
    ...
    if st.button("🚀 2페이지 Gemini AI 분석 코멘트 생성"):
        with st.spinner("분석 중..."):
            p2_summary = (
                f"측정 일시/장소: {m2_date} {m2_loc} / 전체평균: {total_avg:.2f} / "
                f"이상치 폐기: {ex_count}개 / 보정평균: {ks_avg:.2f} / "
                f"각도 {angle_val}도 적용 최종 반발도: {corrected_R:.2f} / "
                f"초음파속도: {v_mps:.1f} m/s / 재령: {total_days}일 / "
                f"슬럼프: {val_slump} mm / 융합추정강도: {fc_final_hybrid:.1f} MPa"
            )
    ...
    st.caption("버튼을 누르면 현재 연산된 수식, 데이터, 학술 출처가 매핑된 2페이지 전용 PDF 보고서가 생성됩니다.")
    ...
    p2_pdf_data = {
        ...
        "fck": fck,
        "angle": angle_val,
        "ultra": f"{v_mps:.1f} m/s" if use_ultra else "없음 (미적용)",
        "slump": f"{val_slump} mm" if use_slump else "없음 (미적용)",
        "r_list": raw_inputs,
        "r_mean": total_avg,
        "r_corr": corrected_R,
        "fc_reb": fc_rebound,
        "fc_ultra": fc_ultra_only,
        "fc_hybrid": fc_final_hybrid
    }
