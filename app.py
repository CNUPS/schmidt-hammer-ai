...
def get_kma_weather(date_obj, time_obj, nx=60, ny=127):
    ...
    기상청 API 키: CX9P4xFMQVy_T-MRTAFcRw
    """
    ...
    # 실패 시 월별 디폴트 기온 매핑 테이블 사용 (안정성 보장)
...
def judge_measurement_possibility(temp, hum):
    if 5.0 <= temp <= 40.0 and hum <= 85.0:
        return True, "🟢 현장 온습도 조건 충족: 정밀 진단 가능 (KS F 2730 규격 준수)"
    else:
        return False, "🔴 현장 기후 조건 부적합: 측정 비권장 또는 환경 요인 추가 보정 필요"
...
def get_gemini_comment(report_text):
    ...
    for delay in [1, 2, 4]:
        try:
            ...
            pass
    return "실시간 종합 진단 결과가 성공적으로 도출되었습니다. 표면 상태 분석 데이터를 기반으로 안정 등급을 충족합니다."
...
with tab1:
    ...
    with col_l:
        ...
        # [특허 기술] 버튼을 누르지 않아도 날짜/시간 기반 자동 날씨 갱신 연동
    ...
    with col_r:
        ...
        # [요청 추가] 자체 학습된 AI 모델 배치 및 지정해주신 학습 사이트 링크 표시
        use_custom_ai = st.checkbox("🎯 자체학습된 AI 모델 적용 (Roboflow - Concrete Defect)", value=False)
        st.caption("🔑 API Key: `wk4BcUKf1InnR2LjHPF8` / `rf_qOwoVElhsYOF2AuBylYjjPwsAjg2` | [자체학습 AI 모델 프로젝트 페이지 바로가기](https://app.roboflow.com/-ovfhd/concrete_defect-j9nuw/train)")
    ...
    if uploaded_file:
        ...
        with st.spinner("AI 서버 모델 결합 분석 및 표면 신뢰도 맵핑 중..."):
            ...
            # 추천 타격 지점 시각화 (PILImage 상에 번호 매핑)
        ...
        with view_col1:
            st.image(p1_map_img_rgb, caption="[왼쪽] 표면 신뢰도 맵핑 결과 (초록: 안전면, 빨강: 손상 및 외곽 3cm 차단영역)", use_container_width=True)
        with view_col2:
            st.image(p1_recommend_img_rgb, caption="[오른쪽] 추천 슈미트헤머 타격 그리드 패턴 (3cm 정격 이격 및 안전영역 배정)", use_container_width=True)
